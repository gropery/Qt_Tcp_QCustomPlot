...
{
    ui->setupUi(this);
    setWindowTitle("Qt TcpServer Plot");
    ...
    ui->comboBoxClientIp->addItem(localIP);
    tcpSocket = new QTcpSocket(this); //创建tcpClient变量
...
void MainWindow::slot_tcpSocket_connected()
{
    ...
    ui->comboBoxServerIp->setEnabled(false);
    ui->spinBoxServerPort->setEnabled(false);
...
void MainWindow::slot_tckSocket_disonnected()
{
    ...
    ui->comboBoxServerIp->setEnabled(true);
    ui->spinBoxServerPort->setEnabled(true);
    ui->comboBoxClientIp->setEnabled(true);
    ui->spinBoxClientPort->setEnabled(true);
...
{
    QString     IP=ui->comboBoxServerIp->currentText(); //IP地址
    quint16     port=ui->spinBoxServerPort->value();    //端口
...
//数据接收
void MainWindow::slot_tcpSocket_readyRead(void)
{
    QByteArray baRecvData;
    QString str;
    QByteArray ba;

    baRecvData = tcpSocket->readAll();
